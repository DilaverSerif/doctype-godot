...
uint32_t simple_upper(uint32_t cp, bool turkish)
{
    ...
    // ı -> I in every locale. U+0130/U+0131 are the one Latin Extended-A
    // pair that are not each other's case partner: İ is a capital with a
    // dot, ı a lowercase without one, so the parity rule below would swap
    // them. (LHU PATCH for Godot: was Turkish-only.)
    if(cp == 0x0131)
    {
        return 'I';
    }
...
uint32_t simple_lower(uint32_t cp, bool turkish)
{
    ...
    // İ -> i in every locale; the full Unicode answer outside Turkish is
    // "i̇" (i + combining dot), which no game UI wants. See simple_upper.
    if(cp == 0x0130)
    {
        return 'i';
    }
